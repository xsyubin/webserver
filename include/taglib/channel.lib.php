...
function lib_channel(&$ctag,&$refObj)
{
    ...
    if($type=='top')
    {
        $sql = "SELECT id,typename,entypename,typebanner,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row=$dsql->GetArray())
            {
                ...
                if( ($row['id']==$typeid || ($topid==$row['id'] && $type=='top') ) && $currentstyle!='' )
                {
					...
					$linkOkstr = str_replace("~entypename~",$row['entypename'],$linkOkstr);
                    $likeType .= $linkOkstr;
                }
                else
                {
